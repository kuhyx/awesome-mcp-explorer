 ...
 *
 * Ported near-verbatim from `~/dufs-cloud/web/src/lib/quantile.ts`, where it was
 * built for file-size and video-duration sliders. It applies here for the same
 * reason and even more strongly: star counts in this dataset run from 1 to
 * ~59,000, so a linear slider's midpoint sits around 30,000 stars — above
 * essentially every server in the list, making the control useless. Mapping
 * through the actual distribution puts the middle of the track at the median.
 */
